...
func TestTheNavigatorGoesBackOnTopOfAHomeWindowThatLostIt(t *testing.T) {
	...
	if _, err := tmuxCommand("set", "-w", "-t", f[0], "@conn_home", "1", ";",
		"set", "-g", "main-pane-height", "3"); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(out, h.pane+" 0 3 1") || !strings.Contains(out, f[1]+" 4 ") {
		t.Errorf("panes:\n%s\nwant the navigator 3 rows tall on top and the buffer under its heading row", out)
	}
}
