...
func TestDockerNotAnsweringIsSaidOnce(t *testing.T) {
	...
	next, _ := m.Update(dockerMsg{stalled: true})
	m = next.(model)
	if !strings.Contains(m.status, "not answering") || !m.dockerStalled {
		t.Errorf("status = %q, stalled %v; want it said the first time", m.status, m.dockerStalled)
	}
	m.status = ""
	next, _ = m.Update(dockerMsg{stalled: true})
	if got := next.(model).status; got != "" {
		t.Errorf("status = %q, want nothing said again while it stays so", got)
	}
}

func TestDockersWordIsMergedWithTheScan(t *testing.T) {
	m := withProcs(80, 12, []Project{{Name: "demo", Path: "/p/demo"}}, nil)
