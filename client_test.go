...
func TestShowingAShellJoinsSwapsOrParks(t *testing.T) {
	cases := []struct {
		...
		{"", "%5", "join-pane -v -d -l 24 -s %5 -t %0 ; select-layout -t %0 main-horizontal"},
		...
		{"%3", "%5", "swap-pane -d -s %5 -t %3 ; resize-window -t %3 -x 120 -y 24"},
		// No buffer wanted: the one there goes back to a window of its own,
		// at the slot's size.
		{"%3", "", "break-pane -d -n shell -s %3 ; resize-window -t %3 -x 120 -y 24"},
	}
	for _, c := range cases {
...
func TestArrangementsCoalesceToTheLastAsked(t *testing.T) {
	...
	mu.Lock()
	defer mu.Unlock()
	if len(moved) != 2 || !strings.Contains(moved[0], "join-pane -v -d -l 24 -s %1 ") || !strings.Contains(moved[1], "join-pane -v -d -l 24 -s %4 ") {
